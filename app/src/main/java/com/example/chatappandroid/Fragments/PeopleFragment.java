...
import com.example.chatappandroid.ViewHolders.UserViewHolder;
            ...
            protected void onBindViewHolder(@NonNull UserViewHolder holder, int position, @NonNull UserModel model) {
                if(!adapter.getRef(position).getKey().equals(FirebaseAuth.getInstance().getCurrentUser())){
                    ...
                    TextDrawable.IBuilder builder = TextDrawable.builder()
                            ...
                            .endConfig()
                            .round();
                    TextDrawable drawable = builder.build(model.getFistName().substring(0,1),color);
                    holder.img_avatar.setImageDrawable(drawable);
                    StringBuilder stringBuilder = new StringBuilder();
                    stringBuilder.append(model.getFistName()).append(" ").append(model.getLastName());
